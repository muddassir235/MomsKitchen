...
import com.momskitchen.momskitchen.Admin.ViewHolders.MenuCreationMealHolder;
 ...
 * interface.
 */
public class MenuCreationMealsList extends Fragment {
    private static final String TAG = "MenuCreationMealsList: ";
    ...
    public static FirebaseRecyclerAdapter<MealItem,MenuCreationMealHolder> lunchListAdapter;
    public static FirebaseRecyclerAdapter<MealItem,MenuCreationMealHolder> dessertListAdapter;
    public static FirebaseRecyclerAdapter<MealItem,MenuCreationMealHolder> complimentListAdapter;

    public static List<MealItem> mealsToAdd = new ArrayList<>();
    public static List<MealItem> mealsToRemove = new ArrayList<>();
     ...
     */
    public MenuCreationMealsList() {
    }

    // TODO: Customize parameter initialization
    @SuppressWarnings("unused")
    public static MenuCreationMealsList newInstance(int position,int columnCount) {
        MenuCreationMealsList fragment = new MenuCreationMealsList();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View view = inflater.inflate(R.layout.fragment_menu_creation_meal_item_list, container, false);
        context = getActivity();
        ...
        if(position == 1){
            ...
            lunchListAdapter = new FirebaseRecyclerAdapter<MealItem, MenuCreationMealHolder>(MealItem.class,R.layout.fragment_menu_creation_meal_item,MenuCreationMealHolder.class,FirebaseDatabase.getInstance().getReference().child("Meals").child("lunch")) {
                @Override
                protected void populateViewHolder(final MenuCreationMealHolder viewHolder,final MealItem model, int position) {
                    viewHolder.mNameTV.setText(model.name);
                    viewHolder.mPriceTV.setText(""+model.pricePerUnit+" PKR");
                    boolean presentOnAllDates = true;
                    for(String date: MenuCreationActivity.dates){
                        if(model.dates!=null) {
                            if (!model.dates.contains(date)) {
                                presentOnAllDates = false;
                            }
                        }else{
                            presentOnAllDates = false;
                        }
                    }
                    if(presentOnAllDates) {
                        if(!mealsToAdd.contains(model)) {
                            mealsToAdd.add(model);
                        }
                        if(mealsToRemove.contains(model)){
                            mealsToRemove.remove(model);
                        }
                    }else{
                        if(!mealsToRemove.contains(model)) {
                            mealsToRemove.add(model);
                        }
                        if (mealsToAdd.contains(model)) {
                            mealsToAdd.remove(model);
                        }
                    }
                    viewHolder.mCheckBox.setChecked(presentOnAllDates);
                    viewHolder.mCheckBox.setChecked(presentOnAllDates);
                    viewHolder.checkedByHand = viewHolder.mCheckBox.isChecked();
                    viewHolder.mCheckBox.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            if(!viewHolder.checkedByHand){
                                viewHolder.checkedByHand = true;
                                if(!mealsToAdd.contains(model)) {
                                    mealsToAdd.add(model);
                                }
                                if(mealsToRemove.contains(model)){
                                ...
                                viewHolder.checkedByHand = false;
                                if(!mealsToRemove.contains(model)) {
                                    mealsToRemove.add(model);
                                }
                                if(mealsToAdd.contains(model)){
            ...
            dessertListAdapter = new FirebaseRecyclerAdapter<MealItem, MenuCreationMealHolder>(MealItem.class,R.layout.fragment_menu_creation_meal_item,MenuCreationMealHolder.class,FirebaseDatabase.getInstance().getReference().child("Meals").child("dessert")) {
                @Override
                protected void populateViewHolder(final MenuCreationMealHolder viewHolder, final MealItem model, int position) {
                    viewHolder.mNameTV.setText(model.name);
                    viewHolder.mPriceTV.setText(""+model.pricePerUnit+" PKR");
            ...
            complimentListAdapter = new FirebaseRecyclerAdapter<MealItem, MenuCreationMealHolder>(MealItem.class,R.layout.fragment_menu_creation_meal_item,MenuCreationMealHolder.class,FirebaseDatabase.getInstance().getReference().child("Meals").child("compliment")) {
                @Override
                protected void populateViewHolder(final MenuCreationMealHolder viewHolder, final MealItem model, int position) {
                    viewHolder.mNameTV.setText(model.name);
                    viewHolder.mPriceTV.setText(""+model.pricePerUnit+" PKR");
